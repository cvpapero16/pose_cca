...
class Plot():
    ...
    def on_click(self, event):
        ...
        self.tmp_x_area.cla()
        self.tmp_x_area.plot(self.scale(data1), "r")
        self.tmp_x_area.plot(self.scale(data2), "b")
        self.tmp_x_area.set_ylim(-5, 5)
    ...
    def on_draw(self, r, x, y):
        ...
        self.tmp_x = x
        self.tmp_y = y
        self.tmp_x_area = self.fig.add_subplot(212)
        ...
        img = self.rho_area.pcolor(X, Y, self.r_m, vmin=-1.0, vmax=1.0, cmap=cm.bwr)#gray
        """
        if self.cbar == None:
...
class CCA(QtGui.QWidget):
    ...
    def corr(self, X, Y, i, j):
        #print X.shape, Y.shape # ex:(34, 5)
        data1 = self.scale(np.fabs(X).mean(axis=1))
        data2 = self.scale(np.fabs(Y).mean(axis=1))

        #print data1
        r = np.corrcoef(data1.T, data2.T)[0][1]
        """
        if r > 0.7:
            print i, j, r
            plt.plot(data1)
